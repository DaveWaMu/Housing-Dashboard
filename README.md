...
We gathered data from various CSV's, transformed and cleaned the data in Jupyter Notebook, and returned a unified dataset. The etl file loads the data to a Heroku SQL database. A flask api returns a JSON formatted dataset with app.py. An app.js file deploys charts, plots, and graphs to individual HTML pages. Machine learning was applied to the data with models utilizing random forest classification and multivariate linear regression.
...
- merge all .csv files by creating a list of the DataFrame names, then use the pandas merge function `pd.merge()`
...
- Michigan State University Data Analytics Bootcamp; August 26, 2021.
